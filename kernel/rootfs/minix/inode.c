...
struct minix_sb_info *sb_bread(object_t dev) {
    ...
    i = (sbi->s_imap_blocks + sbi->s_zmap_blocks) * sizeof(u8 *);
    map = kalloc(i);
    if(!map)
        goto out_no_map;
    sbi->s_imap = &map[0];
    ...
    for(i = 0;i < sbi->s_imap_blocks;i++) {
        sbi->s_imap[i] = kalloc(BLOCK_SIZE);
        if(!(sbi->s_imap[i]))
            goto out_no_bitmap;
        ret = block_rw(dev,IF_READPAGE,sbi->s_imap[i],block,1);
        if(ret)
            goto out_no_bitmap;
        block++;
    }
    ...
    for(i = 0;i < sbi->s_zmap_blocks;i++) {
        sbi->s_zmap[i] = kalloc(BLOCK_SIZE);
        if(!(sbi->s_zmap[i]))
            goto out_no_bitmap;
        ret = block_rw(dev,IF_READPAGE,sbi->s_zmap[i],block,1);
        if(ret)
            goto out_no_bitmap;
        block++;
    }
