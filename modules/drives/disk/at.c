...
static void at_read(object_t caller,void *buffer,off_t offset,cnt_t count) {
    _rw(caller,AT_READ,buffer,offset,count * 2);
}

static void at_write(object_t caller,void *buffer,off_t offset,cnt_t count) {
...
static void at_init(void){
    hook(IF_WRITEPAGE, at_write);
    hook(IF_READPAGE,at_read);
    hook(IF_INTR,_io);
