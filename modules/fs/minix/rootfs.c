...
static int v2_read(struct minix_inode_info *inode,void *buffer,off_t offset,cnt_t count){
    unsigned long  zone = offset / BLOCK_SIZE;
    static char   block[BLOCK_SIZE];
    cnt_t read_count = BLOCK_SIZE - (offset % BLOCK_SIZE);
...
static void minix_read(object_t o,void *buffer,cnt_t count){
    cnt_t read_count;
...
static int v2_write(struct minix_inode_info *inode,void *buffer,off_t offset,cnt_t count){
    cnt_t write_count;
    unsigned long  zone = offset / BLOCK_SIZE;
...
static void minix_write(object_t o,void *buffer,cnt_t count) {
    cnt_t write_count;
    File *file = self()->private_data;
