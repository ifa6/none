...
#ifdef  PRINT_TEXT
static void gputc(int ch);
#endif
#define setPalett(color,index)  ({\
        outb(0xffff,0x3c6);\
        outb(index,0x3c8);\
        outb(color.red >> 2,0x3c9);\
        outb(color.green >> 2,0x3c9);\
        outb(color.blue >> 2,0x3c9);})
...
#ifdef  PRINT_TEXT
static void gputc(int ch){
    unsigned char *_ch = (void*)(0xFFA6E + (ch << 3));
    static int pos = 0;
    int i;
    for(i = 0;i < 8;i++){
        for(int j = 0;j < 8;j++){
            if((_ch[i] >> (8 - j)) & 1){
                drawPixel(0,j + pos,i,40);
                //drawPixel(0,j * 2 + 1,i * 2 + 1,40);
            }
        }
    }
    pos += 8;
